...
mymesh,myort,myjoint = OWENS.create_mesh_struts(;Htwr_base=1e-5,
    Htwr_blds = height, #blade height
    Hbld = height,
...
bld_OWENSPreCompoutput,bld_OWENSPreCompinput = OWENS.getOWENSPreCompOutput(numadIn_bld;plyprops)
sectionPropsArray_bld = OWENS.getSectPropsFromOWENSPreComp(spanpos,numadIn_bld,bld_OWENSPreCompoutput;precompinputs=bld_OWENSPreCompinput)

stiff_bld, mass_bld = OWENS.getSectPropsFromOWENSPreComp(spanpos,numadIn_bld,bld_OWENSPreCompoutput;GX=true)
...
thickness_OWENSPreComp_lag = zeros(length(bld_OWENSPreCompinput))
thickness_OWENSPreComp_flap = zeros(length(bld_OWENSPreCompinput))
for ipc = 1:length(bld_OWENSPreCompinput)
    refY = bld_OWENSPreCompinput[ipc].le_loc*bld_OWENSPreCompinput[ipc].chord
                                # Negative distance for lag, to align with SAND-88-1144
    thickness_OWENSPreComp_lag[ipc] = -(bld_OWENSPreCompinput[ipc].chord-(refY+bld_OWENSPreCompoutput[ipc].y_sc))
    thickness_OWENSPreComp_flap[ipc] = maximum(bld_OWENSPreCompinput[ipc].ynode)*bld_OWENSPreCompinput[ipc].chord - bld_OWENSPreCompoutput[ipc].x_sc
end
# PyPlot.figure()
...
OWENSPreCompoutput,OWENSPreCompinput = OWENS.getOWENSPreCompOutput(numadIn;plyprops)
sectionPropsArray_twr = OWENS.getSectPropsFromOWENSPreComp(LinRange(0,1,nTwrElem),numadIn,OWENSPreCompoutput;precompinputs=OWENSPreCompinput)

stiff_twr, mass_twr = OWENS.getSectPropsFromOWENSPreComp(LinRange(0,1,nTwrElem),numadIn,OWENSPreCompoutput;GX=true)
#Struts
...
stiff_array = [stiff_array;fill(mass_blds[end],Nremain)]
mass_array = [mass_array;fill(mass_blds[end],Nremain)]
...
chord[T1:T4] .= 1.07
chord[T2:T3] .= 0.9191
#TODO: when twist introduced, aero should pull it from the OWENSPreComp input data
...
system, assembly, sections = OWENS.owens_to_gx(mymesh,myort,myjoint,sectionPropsArray,stiff_array,mass_array;VTKmeshfilename="./SNL34m")
...
#
#
# println("1 ei_flap: $(bld_OWENSPreCompoutput[1].ei_flap) exp: $ei_flap_exp_base")
# println("1 ei_lag: $(bld_OWENSPreCompoutput[1].ei_lag) exp: $ei_lag_exp_base")
# println("1 ea: $(bld_OWENSPreCompoutput[1].ea) exp: $ea_exp_base")
# println()
# println("5 ei_flap: $(bld_OWENSPreCompoutput[5].ei_flap) exp: $ei_flap_exp_mid")
# println("5 ei_lag: $(bld_OWENSPreCompoutput[5].ei_lag) exp: $ei_lag_exp_mid")
# println("5 ea: $(bld_OWENSPreCompoutput[5].ea) exp: $ea_exp_mid")
# println()
# println("12 ei_flap: $(bld_OWENSPreCompoutput[12].ei_flap) exp: $ei_flap_exp_center")
# println("12 ei_lag: $(bld_OWENSPreCompoutput[12].ei_lag) exp: $ei_lag_exp_center")
# println("12 ea: $(bld_OWENSPreCompoutput[12].ea) exp: $ea_exp_center")
# println()
...
function runowens(model,feamodel,mymesh,myel,aeroForcesDMS,deformTurb;steady=true,system=nothing,assembly=nothing,VTKFilename="./outvtk")
    ...
    if !steady
        ...
        #     OWENS.OWENSFEA_VTK(VTKFilename,t,uHist,system,assembly,sections;scaling=1,azi=aziHist)
        ...
        displ,elStrain,staticAnalysisSuccessful,FReaction = OWENSFEA.staticAnalysis(feamodel,mymesh,myel,displ,model.OmegaInit,model.OmegaInit,elStorage)
