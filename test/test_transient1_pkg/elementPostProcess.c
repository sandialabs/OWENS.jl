  ...
  Fpp[12])
{
  ...
  memset(&elInput_dispdot_data[0], 0, 12U * sizeof(double));
  memset(&elInput_dispddot_data[0], 0, 12U * sizeof(double));
  ...
  b_index = 1.0;
  elInput_xloc_idx_1_tmp = (int)elementNumber - 1;
  ...
  for (j = 0; j < 2; j++) {
    ...
    for (k = 0; k < 6; k++) {
      ...
      if (c_strcmp(model_analysisType) || b_strcmp(model_analysisType)) {
        aoffset = (int)((d - 1.0) * 6.0 + ((double)k + 1.0)) - 1;
        elx_tmp = (int)b_index - 1;
  ...
  if ((!c_strcmp(model_analysisType)) && (!b_strcmp(model_analysisType))) {
    /* if(strcmp(analysisType,'S')) */
    memcpy(&eldispiter_data[0], &eldisp_data[0], 12U * sizeof(double));
  ...
  aoffset *= elx_tmp;
  if (0 <= aoffset - 1) {
    memcpy(&expl_temp.omegaDotVec.data[0], &elInput_omegaDotVec_data[0], aoffset
           * sizeof(double));
  }
  ...
  if (0 <= aoffset - 1) {
    memcpy(&expl_temp.omegaVec.data[0], &elInput_omegaVec_data[0], aoffset *
           sizeof(double));
  }
  ...
  aoffset = j * k;
  if (0 <= aoffset - 1) {
    memcpy(&expl_temp.accelVec.data[0], &elInput_accelVec_data[0], aoffset *
           sizeof(double));
  }
  ...
  memset(&expl_temp.dispm1.data[0], 0, 12U * sizeof(double));
  memset(&expl_temp.concLoad[0], 0, 12U * sizeof(double));
  memset(&expl_temp.concStiff[0], 0, 12U * sizeof(double));
  ...
  memcpy(&expl_temp.displ_iter.data[0], &eldispiter_data[0], 12U * sizeof(double));
  memcpy(&expl_temp.dispddot.data[0], &elInput_dispddot_data[0], 12U * sizeof
         (double));
  memcpy(&expl_temp.dispdot.data[0], &elInput_dispdot_data[0], 12U * sizeof
         (double));
  memcpy(&expl_temp.disp.data[0], &eldisp_data[0], 12U * sizeof(double));
  ...
  if (0 <= elInput_analysisType_size_idx_1 - 1) {
    memcpy(&expl_temp.analysisType.data[0], &elInput_analysisType_data[0],
  ...
  memcpy(&eldisp_data[0], &b_expl_temp.FhatLessConc[0], 12U * sizeof(double));
...
/*
 * File trailer for elementPostProcess.c
 *
 * [EOF]
 */
