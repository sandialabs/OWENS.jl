...
static const b_struct_T r = { { 0.0, 0.0 },/* ac */
  { 0.0, 0.0 },                        /* twist */
  { 0.0, 0.0 },                        /* rhoA */
  { 0.0, 0.0 },                        /* EIyy */
  { 0.0, 0.0 },                        /* EIzz */
  ...
  emxArray_real_T *el_roll, emxArray_boolean_T *el_rotationalEffects)
{
  ...
  for (n = 0; n < loop_ub; n++) {
    getSplitLine(fileid, el_elLen);

    /* read element data */
    getSplitLine(fileid, el_psi);
    ...
    y[0] = fabs(el_props->data[n].EIyy[0] - el_props->data[n].EIzz[0]);
    y[1] = fabs(el_props->data[n].EIyy[1] - el_props->data[n].EIzz[1]);
    b_y = true;
    k = 0;
    exitg1 = false;
    while ((!exitg1) && (k < 2)) {
  ...
  for (n = 0; n < i; n++) {
    el_elLen->data[(int)bladeData_struct_nodeNum_data[n] - 1] =
  ...
  for (n = 0; n < i; n++) {
    if (c_bladeData_struct_elementNum_d[n] != -1.0) {
      el_props->data[(int)c_bladeData_struct_elementNum_d[n] - 1].b[0] = 0.5 *
        el_elLen->data[(int)bladeData_struct_nodeNum_data[n] - 1];
      el_props->data[(int)c_bladeData_struct_elementNum_d[n] - 1].b[1] = 0.5 *
        el_elLen->data[(int)bladeData_struct_nodeNum_data[n + 1] - 1];

      /* element semi chord */
      idx = n + bladeData_struct_remaining_size[0] * 11;
      el_props->data[(int)c_bladeData_struct_elementNum_d[n] - 1].a0[0] =
        bladeData_struct_remaining_data[idx];
      el_props->data[(int)c_bladeData_struct_elementNum_d[n] - 1].a0[1] =
      ...
      idx = (int)c_bladeData_struct_elementNum_d[n] - 1;

      /* convert "ac" to semichord fraction foreward of halfchord */
      el_props->data[(int)c_bladeData_struct_elementNum_d[n] - 1].a[0] =
        ((el_props->data[idx].a[0] + 0.25 * (2.0 * el_props->data[idx].b[0])) -
         el_props->data[idx].b[0]) / el_props->data[idx].b[0];
      el_props->data[(int)c_bladeData_struct_elementNum_d[n] - 1].a[1] =
        ((el_props->data[idx].a[1] + 0.25 * (2.0 * el_props->data[idx].b[1])) -
         el_props->data[idx].b[1]) / el_props->data[idx].b[1];
      el_props->data[(int)c_bladeData_struct_elementNum_d[n] - 1].ac[0] =
        el_props->data[idx].ac[0] * 2.0;
      el_props->data[(int)c_bladeData_struct_elementNum_d[n] - 1].ac[1] =
        el_props->data[idx].ac[1] * 2.0;

      /* physical aero center offset from elastic axis */
      el_props->data[(int)c_bladeData_struct_elementNum_d[n] - 1].
...
/*
 * File trailer for readElementData.c
 *
 * [EOF]
 */
