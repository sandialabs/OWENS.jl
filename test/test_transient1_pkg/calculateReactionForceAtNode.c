/*
 * File: calculateReactionForceAtNode.c
 *
...
void calculateReactionForceAtNode(const char model_analysisType[3], double
  ...
  *timeInt, const g_struct_T dispData, const emxArray_real_T *displ_iter, double
  Omega, double OmegaDot, const double CN2H[9], double cummulativeForce[6])
{
  boolean_T b;
  int i;
  ...
  for (i = 0; i < b_i; i++) {
    ...
    for (i1 = 0; i1 < 6; i1++) {
      cummulativeForce[i1] += Fpp[(int)(b_elListLocalNodeNumbers + ((double)i1 +
        1.0)) - 1];
    }
  }
...
/*
 * File trailer for calculateReactionForceAtNode.c
 *
 * [EOF]
 */
