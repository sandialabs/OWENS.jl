/*
 * File: calculateStrainForElements.c
 *
  ...
  *elStrain)
{
  ...
  for (i = 0; i < loop_ub; i++) {
    ...
    memset(&elInput_disp_data[0], 0, 12U * sizeof(double));
    for (aoffset = 0; aoffset < 2; aoffset++) {
      /* define element coordinates and displacements associated with element */
      for (k = 0; k < 6; k++) {
        elInput_disp_data[b_index] = displ->data[(int)((conn->data[i +
          conn->size[0] * aoffset] - 1.0) * 6.0 + ((double)k + 1.0)) - 1];
        b_index++;
      }
    }
...
/*
 * File trailer for calculateStrainForElements.c
 *
 * [EOF]
 */
