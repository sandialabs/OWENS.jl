...
void b_importCactusFile(const emxArray_char_T *filename, emxArray_real_T *data)
{
  ...
  do {
    ...
    if (!(b_x != 0.0)) {
      ...
      loop_ub = ii->size[0];
      for (i = 0; i < loop_ub; i++) {
        delimiter_idx->data[i + 1] = (unsigned int)ii->data[i];
      }
      ...
      for (kstr = 0; kstr <= i - 2; kstr++) {
        ...
        b_data[((int)j + 2002 * kstr) - 1] = dc.re;
      }
    } else {
      exitg1 = 1;
    }
...
/*
 * File trailer for importCactusFile.c
 *
 * [EOF]
 */
