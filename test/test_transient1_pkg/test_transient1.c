...
void test_transient1(void)
{
  ...
  double v_data[36];
  double b_dv1[2];
  if (isInitialized_test_transient1 == false) {
    test_transient1_initialize();
  }
  ...
  /*  ********************************************************************* */
  memset(&cmkValues[0], 0, 72U * sizeof(double));
  for (idx = 0; idx < 6; idx++) {
    /*  set up mass matrix */
    jj = idx + 6 * idx;
    cmkValues[jj] = uv[idx];
  ...
  for (ii = 0; ii < idx; ii++) {
    ...
    if (!(filestar == b_NULL)) {
      fprintf(filestar, "%.0f %s %.0f %.0f %.2f\n", 1.0, "M6", (double)i_data[ii],
              (double)j_data[ii], v_data[ii]);
      if (autoflush) {
        fflush(filestar);
      }
    }
...
/*
 * File trailer for test_transient1.c
 *
 * [EOF]
 */
