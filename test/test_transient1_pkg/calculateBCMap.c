                    ...
                    emxArray_real_T *reducedDofList, emxArray_real_T *bcMap)
{
  ...
  for (b_i = 0; b_i < i; b_i++) {
    ...
    while ((!exitg1) && (k <= constrainedDof->size[0] - 1)) {
      absx = fabs(constrainedDof->data[k] / 2.0);
      ...
      if ((fabs(constrainedDof->data[k] - a) < absx) || (rtIsInf(a) && rtIsInf
           (constrainedDof->data[k]) && ((a > 0.0) == (constrainedDof->data[k] >
             0.0)))) {
        tf = true;
        exitg1 = true;
      } else {
...
/*
 * File trailer for calculateBCMap.c
 *
 * [EOF]
 */
