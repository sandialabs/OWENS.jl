  ...
  emxArray_real_T *structuralElNumbers)
{
  ...
  numNodesPerBlade = (double)(loop_ub + 1) / numBlades;
  ...
  for (b_i = 0; b_i < i; b_i++) {
    d = (((double)b_i + 1.0) - 1.0) * numNodesPerBlade + 1.0;
    d1 = ((double)b_i + 1.0) * numNodesPerBlade;
    if (d > d1) {
      i1 = 0;
      i2 = 0;
    } else {
    ...
    idx = (int)(((double)b_i + 1.0) * numNodesPerBlade);
  ...
  if (0 <= loop_ub) {
    memcpy(&bladeData_bladeNum_data[0], &a_data[0], (loop_ub + 1) * sizeof
           (double));
  }
  ...
  time_size[1], emxArray_real_T *ForceValHist, emxArray_real_T *ForceDof)
{
  ...
  numAeroTS = (double)data->size[0] / numAeroEl;
  ...
  for (b_i = 0; b_i < i; b_i++) {
    ...
    m = (int)data->data[b_i + data->size[0] * 2] - 1;
    n = (int)data->data[b_i + data->size[0] * 3] - 1;
  ...
  i2 = (int)cactusGeom_blade->data[0].NElem;
  ...
  for (b_i = 0; b_i < i; b_i++) {
    for (j = 0; j < cactusGeom_NBlade; j++) {
      i1 = (int)cactusGeom_blade->data[j].NElem;
      for (k = 0; k < i1; k++) {
        /*             %% */
        ...
        bladeForce->data[j].M25->data[b_i + bladeForce->data[j].M25->size[0] * k]
          = M25perSpan_data[n];
        b_index = (unsigned int)((int)b_index + 1);
      }
    }
  }
  ...
  for (b_i = 0; b_i < cactusGeom_NBlade; b_i++) {
    numAeroEl = cactusGeom_blade->data[b_i].QCy->data[(int)
  ...
  for (b_i = 0; b_i < i; b_i++) {
    for (j = 0; j < cactusGeom_NBlade; j++) {
      ...
      for (k = 0; k <= i1 - 2; k++) {
        ...
        for (i2 = 0; i2 < 6; i2++) {
          dofList[i2] = numAeroTS + ((double)i2 + 1.0);
          dofList[i2 + 6] = b_node2 + ((double)i2 + 1.0);
        }
        ...
        elInput_xloc[0] = 0.0;
        elInput_xloc[1] = sqrt((numAeroTS * numAeroTS + numAeroEl * numAeroEl) +
          node2 * node2);
        n = (int)structuralElNumbers->data[j + structuralElNumbers->size[0] * k]
          - 1;
  ...
  for (b_i = 0; b_i < i; b_i++) {
    ...
    if (bladeData_remaining_size[1] != 0) {
      n = spanLocNorm->size[1];
      for (i1 = 0; i1 < n; i1++) {
        ForceValHist->data[((int)b_index + ForceValHist->size[0] * i1) - 1] =
          spanLocNorm->data[b_i + spanLocNorm->size[0] * i1];
      }

      ForceDof->data[(int)b_index - 1] = (double)b_i + 1.0;
      b_index++;
    }
  }
...
/*
 * File trailer for mapCactusLoadsFile.c
 *
 * [EOF]
 */
