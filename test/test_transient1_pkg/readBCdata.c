                ...
                *BC_nummBC, emxArray_real_T *BC_isConstrained)
{
  ...
  for (b_i = 0; b_i < i; b_i++) {
    ...
    loop_ub = ii->size[0];
    for (i1 = 0; i1 < loop_ub; i1++) {
      delimiter_idx->data[i1 + 1] = (unsigned int)ii->data[i1];
    }
    ...
    for (k = 0; k <= i1 - 2; k++) {
      ...
      if ((double)u + 1.0 > (double)u1 - 1.0) {
        idx = 0;
        kstr = 0;
      } else {
        idx = (int)u;
        kstr = (int)((double)u1 - 1.0);
      }
  ...
  for (b_i = 0; b_i < i; b_i++) {
    for (kstr = 0; kstr < 6; kstr++) {
      a = (((double)b_i + 1.0) - 1.0) * 6.0 + ((double)kstr + 1.0);
      b_bool = false;
      k = 0;
      exitg2 = false;
      while ((!exitg2) && (k <= constDof->size[0] - 1)) {
        absx = fabs(constDof->data[k] / 2.0);
        ...
        if (fabs(constDof->data[k] - a) < absx) {
          b_bool = true;
          exitg2 = true;
        } else {
          k++;
      ...
      if (b_bool) {
        isConstrained->data[(int)b_index - 1] = 1;
      }
