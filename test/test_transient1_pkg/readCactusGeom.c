...
static void processLine(double fid, emxArray_real_T *data)
{
  ...
  loop_ub = (int)delimiter_idx_size;
  ...
  for (loop_ub = 0; loop_ub <= n; loop_ub++) {
    if (delimiter_log_idx->data[loop_ub] && delimiter_log_idx->data[loop_ub + 1]
        && (!delimiter_log_idx->data[loop_ub + 2])) {
      delimiter_idx->data[(int)j - 1] = (unsigned int)(loop_ub + 2);
      j++;
    }
  }
                    ...
                    *cactusGeom_strut)
{
  ...
  *cactusGeom_NStrut = NStrut->data[0];
  processLine(fileid, cactusGeom_RotN);
  processLine(fileid, cactusGeom_RotP);
  processLine(fileid, cactusGeom_RefAR);
  processLine(fileid, cactusGeom_RefR);
  b_myfgetl(fileid);
  ...
  /*  skip a line */
  processLine(fileid, NElem);
  loop_ub = (int)(NElem->data[0] + 1.0);
  i = blade_QCx->size[0];
  ...
  if (0 <= NBlade->data[0] - 1) {
    ...
    if (1.0 > NElem->data[0] + 1.0) {
      ...
      d_loop_ub = (int)(NElem->data[0] + 1.0);
    }

    if (1.0 > NElem->data[0] + 1.0) {
      e_loop_ub = 0;
    } else {
      e_loop_ub = (int)(NElem->data[0] + 1.0);
    }

    if (1.0 > NElem->data[0] + 1.0) {
      f_loop_ub = 0;
    } else {
      f_loop_ub = (int)(NElem->data[0] + 1.0);
    }

    if (1.0 > NElem->data[0] + 1.0) {
      g_loop_ub = 0;
    } else {
      g_loop_ub = (int)(NElem->data[0] + 1.0);
    }

    if (1.0 > NElem->data[0] + 1.0) {
      h_loop_ub = 0;
    } else {
      h_loop_ub = (int)(NElem->data[0] + 1.0);
    }

    if (1.0 > NElem->data[0]) {
      i_loop_ub = 0;
    } else {
      i_loop_ub = (int)NElem->data[0];
    }

    if (1.0 > NElem->data[0]) {
      j_loop_ub = 0;
    } else {
      j_loop_ub = (int)NElem->data[0];
    }

    if (1.0 > NElem->data[0]) {
      k_loop_ub = 0;
    } else {
      k_loop_ub = (int)NElem->data[0];
    }

    if (1.0 > NElem->data[0]) {
      l_loop_ub = 0;
    } else {
      l_loop_ub = (int)NElem->data[0];
    }

    if (1.0 > NElem->data[0]) {
      m_loop_ub = 0;
    } else {
      m_loop_ub = (int)NElem->data[0];
    }

    if (1.0 > NElem->data[0]) {
      n_loop_ub = 0;
    } else {
      n_loop_ub = (int)NElem->data[0];
    }

    if (1.0 > NElem->data[0]) {
      o_loop_ub = 0;
    } else {
      o_loop_ub = (int)NElem->data[0];
    }

    if (1.0 > NElem->data[0]) {
      p_loop_ub = 0;
    } else {
      p_loop_ub = (int)NElem->data[0];
    }

    if (1.0 > NElem->data[0]) {
      q_loop_ub = 0;
    } else {
      q_loop_ub = (int)NElem->data[0];
    }

    if (1.0 > NElem->data[0]) {
      r_loop_ub = 0;
    } else {
      r_loop_ub = (int)NElem->data[0];
    }

    if (1.0 > NElem->data[0]) {
      s_loop_ub = 0;
    } else {
      s_loop_ub = (int)NElem->data[0];
    }

    if (1.0 > NElem->data[0]) {
      t_loop_ub = 0;
    } else {
      t_loop_ub = (int)NElem->data[0];
    }

    if (1.0 > NElem->data[0]) {
      u_loop_ub = 0;
    } else {
      u_loop_ub = (int)NElem->data[0];
    }

    if (1.0 > NElem->data[0]) {
      v_loop_ub = 0;
    } else {
      v_loop_ub = (int)NElem->data[0];
    }

    if (1.0 > NElem->data[0]) {
      w_loop_ub = 0;
    } else {
      w_loop_ub = (int)NElem->data[0];
    }

    if (1.0 > NElem->data[0]) {
      x_loop_ub = 0;
    } else {
      x_loop_ub = (int)NElem->data[0];
    }
  }
...
/*
 * File trailer for readCactusGeom.c
 *
 * [EOF]
 */
