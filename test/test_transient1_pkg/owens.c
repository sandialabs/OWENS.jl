...
static const char cv[73] = { '.', '/', 'i', 'n', 'p', 'u', 't', '_', 'f', 'i',
  'l', 'e', 's', '_', 't', 'e', 's', 't', '/', '1', '_', 'F', 'o', 'u', 'r', 'C',
  'o', 'l', 'u', 'm', 'n', 'S', 'e', 'm', 'i', '_', '2', 'n', 'd', 'P', 'a', 's',
...
static void generateOutputFilename(char outputfilename_data[], int
  ...
  *BC_isConstrained, emxArray_real_T *redVectorMap)
{
  ...
  for (b_i = 0; b_i < i; b_i++) {
    for (loop_ub = 0; loop_ub < 6; loop_ub++) {
      if (!(BC_isConstrained->data[(int)((((double)b_i + 1.0) - 1.0) * 6.0 +
            ((double)loop_ub + 1.0)) - 1] != 0.0)) {
  ...
  for (b_i = 0; b_i < i; b_i++) {
    for (loop_ub = 0; loop_ub < 6; loop_ub++) {
      d = (((double)b_i + 1.0) - 1.0) * 6.0 + ((double)loop_ub + 1.0);
      if (!(BC_isConstrained->data[(int)d - 1] != 0.0)) {
        dofList->data[(int)b_index - 1] = d;
  ...
  for (b_i = 0; b_i < i; b_i++) {
    ...
    while ((!exitg1) && (loop_ub <= bcdoflist->size[0] - 1)) {
      b_index = fabs(bcdoflist->data[loop_ub] / 2.0);
      if ((!rtIsInf(b_index)) && (!rtIsNaN(b_index))) {
        if (b_index <= 2.2250738585072014E-308) {
          b_index = 4.94065645841247E-324;
        } else {
          frexp(b_index, &exponent);
          b_index = ldexp(1.0, exponent - 53);
        }
      } else {
        b_index = rtNaN;
      }

      if (fabs(bcdoflist->data[loop_ub] - ((double)b_i + 1.0)) < b_index) {
        b_tf = true;
        exitg1 = true;
      } else {
        loop_ub++;
    ...
    if (b_tf) {
      ...
      na = dofList->size[1];
      unnamed_idx_1 = (unsigned int)dofList->size[1];
      ...
      for (loop_ub = 0; loop_ub < na; loop_ub++) {
        ...
        if (fabs(((double)b_i + 1.0) - dofList->data[loop_ub]) < ldexp(1.0,
             b_exponent - 53)) {
  ...
  outputfilename_size[2])
{
  ...
  if (0 <= idx - 1) {
    memcpy(&outputfilename_data[0], &cv[0], idx * sizeof(char));
  }
...
void owens(const double varargin_7[2])
{
  ...
  if (0 <= loop_ub - 1) {
    memcpy(&fdirectory_data[0], &cv[0], loop_ub * sizeof(char));
  }
  ...
  myfgetl(fileid, varargin_2);
  if (loop_ub != 0) {
    input_sizes_idx_1 = (signed char)loop_ub;
  } else {
    input_sizes_idx_1 = 0;
  }
  ...
  /* mesh file name */
  myfgetl(fileid, b_varargin_2);
  if (loop_ub != 0) {
    b_input_sizes_idx_1 = (signed char)loop_ub;
  } else {
    b_input_sizes_idx_1 = 0;
  }

  /* element data file name */
  myfgetl(fileid, c_varargin_2);
  if (loop_ub != 0) {
    c_input_sizes_idx_1 = (signed char)loop_ub;
  } else {
  ...
  myfgetl(fileid, d_varargin_2);
  if (loop_ub != 0) {
    d_input_sizes_idx_1 = (signed char)loop_ub;
  } else {
  ...
  myfgetl(fileid, e_varargin_2);
  if (loop_ub != 0) {
    e_input_sizes_idx_1 = (signed char)loop_ub;
  } else {
  ...
  myfgetl(fileid, f_varargin_2);
  if (loop_ub != 0) {
    f_input_sizes_idx_1 = (signed char)loop_ub;
  } else {
  ...
  myfgetl(fileid, g_varargin_2);
  if (loop_ub != 0) {
    g_input_sizes_idx_1 = (signed char)loop_ub;
  } else {
  ...
  if (delimiter_idx->data[0] + 1.0 > delimiter_idx->data[1] - 1.0) {
    ...
    i = (int)(delimiter_idx->data[0] + 1.0) - 1;
    i1 = (int)(delimiter_idx->data[1] - 1.0);
  }
  ...
  if (delimiter_idx->data[1] + 1.0 > i) {
    ...
    i1 = (int)(delimiter_idx->data[1] + 1.0) - 1;
  }
  ...
  for (i = 0; i < 6; i++) {
    model_owensfile->data[i + b_loop_ub] = b_cv[i];
  }
  ...
  myfgetl(fileid, line);
  if (loop_ub != 0) {
    h_input_sizes_idx_1 = (signed char)loop_ub;
  } else {
  ...
  if (loop_ub != 0) {
    unnamed_idx_1 = (signed char)loop_ub;
  } else {
    unnamed_idx_1 = 0;
  }
  ...
  /* generator file name */
  getSplitLine(fileid, delimiter_idx);
  ...
  if (loop_ub != 0) {
    sizes_idx_1 = (signed char)loop_ub;
  } else {
    sizes_idx_1 = 0;
  }
  ...
  constructReducedDispVectorMap(mesh_numNodes, jnt_struct_jointTransform->size[1],
    BC_numpBC, model_BC_pBC, delimiter_idx, b_mesh_numNodes);
