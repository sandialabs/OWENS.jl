/*
 * File: findElementsAssociatedWithNodeNumber.c
 *
  ...
  *localNode)
{
  ...
  if (jointData->size[0] != 0) {
    ...
    for (j = 0; j <= i; j++) {
      ...
      if (fabs(1.0 - jointData->data[j + jointData->size[0] * 2]) <
    ...
    for (j = 0; j <= i; j++) {
      ...
      if ((fabs(nodeNum - d) < b_r) || (rtIsInf(d) && rtIsInf(nodeNum) && ((d >
             0.0) == (nodeNum > 0.0)))) {
        tf->data[j] = true;
      }
    }
    ...
    if (res1->size[0] != 0) {
      i = elList->size[0] * elList->size[1];
      elList->size[0] = 1;
      elList->size[1] = (int)((double)res1->size[0] * (double)conn->size[0]);
      emxEnsureCapacity_real_T(elList, i);
      idx = (int)((double)res1->size[0] * (double)conn->size[0]);
      for (i = 0; i < idx; i++) {
        elList->data[i] = 0.0;
      }

      d = (double)res1->size[0] * (double)conn->size[0];
      i = (int)d;
      ...
      for (j = 0; j < i; j++) {
        ...
        for (exponent = 0; exponent < i1; exponent++) {
          ...
          if ((fabs(s - d) < b_r) || (rtIsInf(d) && rtIsInf(s) && ((d > 0.0) ==
                (s > 0.0)))) {
            res[0] = true;
          }
          ...
          while ((!exitg1) && (b_exponent < 2)) {
            if (res[b_exponent]) {
              idx++;
              ii_data[idx - 1] = (signed char)(b_exponent + 1);
              if (idx >= 2) {
                exitg1 = true;
              } else {
                b_exponent++;
          ...
          if (idx != 0) {
            ...
            idx = (int)b_index - 1;
            elList->data[idx] = (double)exponent + 1.0;
...
/*
 * File trailer for findElementsAssociatedWithNodeNumber.c
 *
 * [EOF]
 */
