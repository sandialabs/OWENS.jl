              ...
              emxArray_real_T *Kg, emxArray_real_T *Fg)
{
  ...
  count = 0;
  memset(&dofList_data[0], 0, 12U * sizeof(double));
  for (i = 0; i < 2; i++) {
    for (j = 0; j < 6; j++) {
      dofList_data[count] = (conn[i] - 1.0) * 6.0 + ((double)j + 1.0);
      count++;
    }
  }
  ...
  for (j = 0; j < 12; j++) {
    count = (int)dofList_data[j] - 1;
    Fg->data[count] += Fe[j];
    for (i = 0; i < 12; i++) {
      b_i = (int)dofList_data[i] - 1;
