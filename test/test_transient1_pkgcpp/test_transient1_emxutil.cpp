...
// an end-user only.
// File: test_transient1_emxutil.cpp
//
...
void emxEnsureCapacity_struct_T1(d_emxArray_struct_T *emxArray, int oldNumel)
{
  ...
  if (newNumel > emxArray->allocatedSize) {
    ...
    newData = std::calloc(static_cast<unsigned int>(i), sizeof(i_struct_T));
    if (emxArray->data != NULL) {
      std::memcpy(newData, emxArray->data, sizeof(i_struct_T) * oldNumel);
      if (emxArray->canFreeData) {
        std::free(emxArray->data);
      }
    }
...
void emxEnsureCapacity_struct_T3(e_emxArray_struct_T *emxArray, int oldNumel)
{
  ...
  if (newNumel > emxArray->allocatedSize) {
    ...
    newData = std::calloc(static_cast<unsigned int>(i), sizeof(j_struct_T));
    if (emxArray->data != NULL) {
      std::memcpy(newData, emxArray->data, sizeof(j_struct_T) * oldNumel);
      if (emxArray->canFreeData) {
        std::free(emxArray->data);
      }
    }
...
//
// File trailer for test_transient1_emxutil.cpp
//
// [EOF]
//
