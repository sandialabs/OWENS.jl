...
for i = 0:length(t)
    if i == 0
        line = ['t,aziHist,OmegaHist,OmegaDotHist,gbHist,gbDotHist,gbDotDotHist,FReactionHist1,FReactionHist2,FReactionHist3,FReactionHist4,FReactionHist5,FReactionHist6,rigidDof,genTorque,genPower,torqueDriveShaft' newline'];
    else
        line = [num2str(t(i)), ',',...
            num2str(aziHist(i)), ',',...
            num2str(OmegaHist(i)), ',',...
            num2str(OmegaDotHist(i)), ',',...
            num2str(gbHist(i)), ',',...
            num2str(gbDotHist(i)), ',',...
            num2str(gbDotDotHist(i)), ',',...
            num2str(FReactionHist(i,1)), ',',...
            num2str(FReactionHist(i,2)), ',',...
            num2str(FReactionHist(i,3)), ',',...
            num2str(FReactionHist(i,4)), ',',...
            num2str(FReactionHist(i,5)), ',',...
            num2str(FReactionHist(i,6)), ',',...
            num2str(rigidDof(i)), ',',...
            num2str(genTorque(i)), ',',...
            num2str(genPower(i)), ',',...
            num2str(torqueDriveShaft(i)), newline ];
    end
...
for i = 0:length(t)
    if i == 0
        ...
        line = [num2str(t(i)), ' ', num2str(uHist(:,i)'), newline ];
    end
    fwrite(fid,line,'char');
end
...
for i = 0:length(t)-1
    if i == 0
        line = ['n_t n_elem' newline];
        fwrite(fid,line,'char');

        line = [num2str(length(t)) ' ' num2str(length(strainHist(:,1))) newline];
        fwrite(fid,line,'char');
    else
        ...
        for j = 1:length(strainHist(:,1))
            line = ['t elem', newline];
            fwrite(fid,line,'char');
            line = [num2str(t(i)), ' ', num2str(j), newline];
            fwrite(fid,line,'char');

            line = ['eps_xx_0 ' num2str(strainHist(j,i).eps_xx_0) newline];
            fwrite(fid,line,'char');
            line = ['eps_xx_z ' num2str(strainHist(j,i).eps_xx_z) newline];
            fwrite(fid,line,'char');
            line = ['eps_xx_y ' num2str(strainHist(j,i).eps_xx_y) newline];
            fwrite(fid,line,'char');
            line = ['gam_xz_0 ' num2str(strainHist(j,i).gam_xz_0) newline];
            fwrite(fid,line,'char');
            line = ['gam_xz_y ' num2str(strainHist(j,i).gam_xz_y) newline];
            fwrite(fid,line,'char');
            line = ['gam_xy_0 ' num2str(strainHist(j,i).gam_xy_0) newline];
            fwrite(fid,line,'char');
            line = ['gam_xy_z ' num2str(strainHist(j,i).gam_xy_z) newline];
            fwrite(fid,line,'char');
        end
    end
end
