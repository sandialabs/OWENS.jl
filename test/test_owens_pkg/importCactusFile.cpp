...
void b_importCactusFile(const emxArray_char_T *filename, emxArray_real_T *data)
{
  ...
  do {
    ...
    if (!(b_x != 0.0)) {
      ...
      for (k = 0; k <= i - 2; k++) {
        ...
        b_data[(static_cast<int>(j) + 2002 * k) - 1] = dc.re;
      }
    } else {
      exitg1 = 1;
    }
  ...
  for (i = 0; i < 22; i++) {
    for (idx = 0; idx < loop_ub; idx++) {
      data->data[idx + data->size[0] * i] = b_data[idx + 2002 * i];
    }
  }
                      ...
                      data_size[2])
{
  ...
  do {
    ...
    if (!(b_x != 0.0)) {
      ...
      for (k = 0; k <= i - 2; k++) {
        ...
        dc = c_str2double(b_line);
        data[(static_cast<int>(j) + 60 * k) - 1] = dc.re;
      }
    } else {
      exitg1 = 1;
    }
  ...
  for (i = 0; i < 16; i++) {
    for (idx = 0; idx < loop_ub; idx++) {
      data_data[idx + loop_ub * i] = data[idx + 60 * i];
    }
  }
