  ...
  *transMatrix)
{
  ...
  d_sparse_mtimes(y_d, y_colidx, y_rowidx, y_m, Kg_d, Kg_colidx, Kg_rowidx, Kg_n,
                  t0_d, counts, t0_rowidx, &outridx, &cend);
  d_sparse_mtimes(t0_d, counts, t0_rowidx, outridx, transMatrix_d,
  ...
  *imagCompSign)
{
  ...
  for (b_i = 0; b_i < i; b_i++) {
    ...
    c_calculateTimoshenkoElementNL(elInput_xloc, elInput_sectionProps_twist,
      elInput_sectionProps_rhoA, elInput_sectionProps_EA,
      elInput_sectionProps_zcm, elInput_sectionProps_ycm, el.psi->data[b_i],
      el.theta->data[b_i], el.roll->data[b_i], elInput_concMass, eldisp_data,
      elx_data, elInput_y_data, elInput_z_data, elInput_CN2H,
      model_RayleighAlpha, model_RayleighBeta, &elStorage->data[b_i], &expl_temp);
