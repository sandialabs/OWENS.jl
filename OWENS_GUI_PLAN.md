...
| G4 | Add a local Genie shell that calls the service layer. | In progress on `codex/gui-project-health-shell` | Open-project bootstrap payloads, static workbench bundles, route handlers, and a route catalog are now tested service contracts for Genie to wrap next. |
