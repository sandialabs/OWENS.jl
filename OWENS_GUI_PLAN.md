...
| G6 | Wire RM2 as the first end-to-end GUI workflow. | Pending | Template opens, health passes, generated Julia script is visible, short run can produce a manifest and result summary. |
| G7 | Add "export generated Julia script" before adding more UI controls. | Partially complete | WindIO run specs can already render scripts; Studio project needs route/button integration. |
...
| G9 | Add first GUI fixture project under `examples/gui` or `docs/src/assets/gui`. | Pending | Fixture opens through the Studio project schema and has deterministic health status in tests. |
