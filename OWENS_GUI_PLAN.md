...
| G4 | Add a local Genie shell that calls the service layer. | In progress on `codex/gui-project-health-shell` | Static workbench bundles now write HTML, health YAML, and generated Julia artifacts; Genie can serve those same route outputs next. |
