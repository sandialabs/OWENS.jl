...
GXBeam/BECAS sectional properties can be selected for GXBeam structural runs by setting `sectional_property_source: gxbeam` in `OWENSFEA_Options` or `material.sectional_property_source = :gxbeam` in `SetupOptions`. OWENS still uses the PreComp/Numad inputs as the source of airfoil geometry, material stacks, upper/lower skin sectors, and shear webs, but converts those inputs into GXBeam sectional meshes before computing the 6x6 stiffness and mass matrices. The legacy default is `precomp`, which preserves the historical PreComp scalar-property path.

When the GXBeam sectional backend is selected through componentized setup, each component stores element-aligned `gxBeamSectionalProperties`. These section objects include the mesh, cache, stiffness, mass, and centers needed for post-processing. After component beam strains are populated, OWENS stores `gxBeamSectionalRecovery` with recovered beam and ply strains/stresses for each structural element and time step. Use `recover_gxbeam_sectional_strain(section, generalized_strain, stiffness)` to map one beam strain vector in GXBeam order `[epsilon_x, epsilon_y, epsilon_z, kappa_x, kappa_y, kappa_z]` back through the sectional mesh.
