...
function real_main()
    ...
    #Blades
    NuMad_geom_xlscsv_file = ARGS[1]#"$path/data/SNL34mGeom.csv"
    ...
    eps_x_grav,eps_z_grav,eps_y_grav,kappa_x_grav,kappa_y_grav,kappa_z_grav,t,FReactionHist_grav = runowens(model,feamodel,mymesh,myel,aeroForcesDMS,VAWTAero.deformTurb,Nbld;steady=true)
    ...
    aeroForcesDMS,VAWTAero.deformTurb,Nbld;steady=false,system,assembly,VTKFilename="$path/vtk/NormalOperation")
...
function runowens(model,feamodel,mymesh,myel,aeroForces,deformTurb,Nbld;steady=true,system=nothing,assembly=nothing,VTKFilename="./outvtk")
    ...
    if !steady
        ...
        topModel=feamodel,topMesh=mymesh,topEl=myel,aero=aeroForces,deformAero=deformTurb,system,assembly)
