...
if(staticAnalysisSuccessful)
    ...
    if(j==1) %for the first rotor speed perform a modal analysis with initial guess frequency of zero to obtain frequency estimate
        [freqOrig,damp,phase1,phase2,imagCompSign] = linearAnalysisModal(modelCopy,mesh,el,displ,Omega,elStorage);
    else
    ...
    for i=2:2:model.numModesToExtract %do p-k iteration for flutter analysis of modes of interest
        ...
        converged = false;
        while(~converged)
        [freq,damp,phase1,phase2,imagCompSign] = linearAnalysisModal(model,mesh,el,displ,Omega,elStorage); %performs modal analysis during iteration
