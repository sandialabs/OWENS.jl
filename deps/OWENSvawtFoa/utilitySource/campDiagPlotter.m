...
%      output:            (NONE)
    ...
    figure%ble: close all;             %close all figure windows
    [r,c]=size(freqMat);
...
%     colorstr = {'b','g','m','c','y','b','g','m','c','y','b','g','m','c','y','b','g','m','c','y'};
    hold on
    
    for i=2:2:numModesToPlot
%            plot(rotorSpeedArray*60,freqMat(:,i),colorstr{i/2},'LineWidth',2); %plot mode i at various rotor speeds
           plot(rotorSpeedArray*60,freqMat(:,i),'LineWidth',2); %plot mode i at various rotor speeds
           hold on;
    end
