function runOWENSWINDIO(modelopt,windio,path)
    ...
    if DLCs != "none"
        tocp, Omegaocp, turbineStartup, generatorOn, useGeneratorFunction = handle_control_strategy(
            controlStrategy,
            normalRPM,
            slowRPM,
    ...
    mass_breakout_blds,mass_breakout_twr,system,assembly,sections,AD15bldNdIdxRng, AD15bldElIdxRng = OWENS.setupOWENS(path;setup_options=setup_opts)
    ...
    OWENS.OWENSVTK(modelopt.OWENS_Options.VTKsaveName,t,uHist,system,assembly,sections,aziHist,mymesh,myel,
    ...
    mymesh,myel,myort,setup_opts.blade.B,epsilon_x_hist,kappa_y_hist,kappa_z_hist,epsilon_z_hist,
    kappa_x_hist,epsilon_y_hist;verbosity=modelopt.OWENS_Options.verbosity,
    Twr_LE_U_idx=1,Twr_LE_L_idx=1,delta_t=modelopt.OWENS_Options.delta_t,
...
"""
function handle_control_strategy(controlStrategy, normalRPM, slowRPM, prescribed_time, prescribed_RPM)
