    ...
    while (i<numTS-1) && timeconverged == false # we compute for the next time step, so the last step of our desired time series is computed in the second to last numTS value
        ...
        while ((uNorm > TOL || aziNorm > TOL || gbNorm > TOL) && (numIterations < MAXITER))
            ...
            if (i==numTS-1 || timeconverged == true) && inputs.analysisType=="TNB" && topModel.nlParams.predef=="update" && (!(uNorm > TOL || platNorm > TOL || aziNorm > TOL || gbNorm > TOL) || (numIterations >= MAXITER))
                GyricFEA.structuralDynamicsTransient(topModel,topMesh,topEl,topdata.topDispData2,topdata.Omega_s,topdata.OmegaDot_s,t[i+1],topdata.delta_t,topElStorage,topdata.topFexternal,Int.(aeroDOFs),topdata.CN2H,topdata.rbData;predef = topModel.nlParams.predef) #TODO topElStorage in topdata
            end
        ...
        if inputs.analysisType=="ROM"
            ...
            topdata.topDispData1 = GyricFEA.DispData(topdata.u_j, topdata.udot_j, topdata.uddot_j, topdata.u_sm1, eta_j, etadot_j, etaddot_j)
        else
            topdata.topDispData1 = GyricFEA.DispData(topdata.u_j, topdata.udot_j, topdata.uddot_j, topdata.u_sm1)
        end
