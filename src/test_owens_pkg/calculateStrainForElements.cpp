...
void calculateStrainForElements(double numEl, const emxArray_real_T *conn, const
  ...
  emxArray_real_T *el_roll, const emxArray_real_T *displ, boolean_T nlflag,
  b_emxArray_struct_T *elStrain)
{
  int b_index;
  int loop_ub;
  ...
  for (i = 0; i < loop_ub; i++) {
    ...
    for (b_index = 0; b_index < 4; b_index++) {
      ...
      // This function interpolates a value using distinct values at valNode
      // and the corresponding shape function N.
      uprime = p_N_x_data[0] * dispLocal[0] + p_N_x_data[1] * dispLocal[6];
      vprime = p_N_x_data[0] * dispLocal[1] + p_N_x_data[1] * dispLocal[7];
      wprime = p_N_x_data[0] * dispLocal[2] + p_N_x_data[1] * dispLocal[8];
      theta_x_prime = p_N_x_data[0] * dispLocal[3] + p_N_x_data[1] * dispLocal[9];
      if (nlflag) {
        elStrain->data[i].eps_xx_0[b_index] = uprime + 0.5 * (wprime * wprime +
          vprime * vprime);
      } else {
        elStrain->data[i].eps_xx_0[b_index] = uprime;
      }
