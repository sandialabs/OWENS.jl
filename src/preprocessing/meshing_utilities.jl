...
#       \
function mesh_beam(;
    L1 = 31.5, #first section of beam length
    ...
    y_shape = nothing,
    z_shape = nothing,
) #offset from 0 before first beam begins
    ...
    myjoint =
        [Float64.(1:1:njoint) jointconn zeros(njoint) zeros(njoint) zeros(njoint) Psi_d_joint Theta_d_joint]
...
# ______
#       \
function mesh_beam_centered(;
    L1 = 6.0, #first section of beam length
    L2 = -6.0, #second section of beam length
    Nelem1 = 2,
    Nelem2 = 2,
    ...
    #Joint Number,   Joint Connections, Joint Type, Joint Mass, Not Used, Psi_D, Theta_D
    myjoint =
        [Float64.(1:1:njoint) Int.(jointconn) zeros(njoint) zeros(njoint) zeros(njoint) Psi_d_joint Theta_d_joint]
...
mutable struct Component
    ...
    strain_U::Any
    strain_L::Any
    ultsafetyfactor_U::Any
    ultsafetyfactor_L::Any
    bucksafetyfactor_U::Any
    bucksafetyfactor_L::Any
    damage_U::Any
    damage_L::Any
    mass::Any
    cost::Any
...
function unifyComponents(components)
    ...
    for itime = 1:N_ts
        if size(new_component.stress_U[1, :, :, 1])[2]==1
            spline_U = OWENS.safeakima(
                spl_x_pts,
                new_component.stress_U[itime, :, 1, 1],
                LinRange(0, 1, N_Nodes),
            )
            spline_L = reverse(
                OWENS.safeakima(
                    spl_x_pts,
                    new_component.stress_L[itime, :, 1, 1],
                    LinRange(0, 1, N_Nodes),
                ),
            )
            ...
            spline_U = FLOWMath.interp2d(
                FLOWMath.akima,
                spl_x_pts,
                spl_y_pts_U,
                new_component.stress_U[itime, :, :, 1],
                LinRange(0, 1, N_Nodes),
                LinRange(0, 1, 100),
            )
            spline_L = reverse(
                FLOWMath.interp2d(
                    ...
                    spl_x_pts,
                    spl_y_pts_L,
                    new_component.stress_L[itime, :, :, 1],
                    LinRange(0, 1, N_Nodes),
                    LinRange(0, 1, 99),
                ),
                dims = 2,
    ...
    for component in components[2:end]
        ...
        new_component.input_materials =
            vcat(new_component.input_materials, component.input_materials)
        new_component.preCompInput =
            vcat(new_component.preCompInput, component.preCompInput)
        new_component.preCompOutput =
            vcat(new_component.preCompOutput, component.preCompOutput)
        ...
        new_component.stiff_matrix =
            vcat(new_component.stiff_matrix, component.stiff_matrix)
        new_component.mass_matrix = vcat(new_component.mass_matrix, component.mass_matrix)
        new_component.sectionProps =
            vcat(new_component.sectionProps, component.sectionProps)
        ...
        for itime = 1:N_ts
            if size(component.stress_U[1, :, :, 1])[2]==1
                spline_U = OWENS.safeakima(
                    spl_x_pts,
                    component.stress_U[itime, :, 1, 1],
                    LinRange(0, 1, N_Nodes),
                )
                spline_L = reverse(
                    OWENS.safeakima(
                        spl_x_pts,
                        component.stress_L[itime, :, 1, 1],
                        LinRange(0, 1, N_Nodes),
                    ),
                )
...
function create_mesh_struts(;
    ...
    if bshapex == zeros(nbelem+1)
        bld_Y = R .* (1.0 .- 4.0 .* (bld_Z/Hbld .- 0.5) .^ 2)
    else
    ...
    for ibld = 1:nblade
        ...
        components = [
            ...
            Component(;
                name = "blade$ibld",
                nodeNumbers = collect(b_botidx[ibld]:b_topidx[ibld]),
            )
        ]
    ...
    function line_addition(
        ...
        if AD15hubR > 1e-6
            ...
            if verbosity>0
                println(
                    "Hub crossing at idx $hubIdx and radially at $R_temp with AD15 hub radius of $AD15hubR",
                )
                println(
                    "Moving strut point from [$(s_x[hubIdx]),$(s_y[hubIdx]),$(s_z[hubIdx])] to [$(s_x[hubIdx]),$(s_y[hubIdx]),$(s_z[hubIdx])]",
                )
            end
    ...
    for istrut = 1:Nstrut
        for ibld = 1:nblade
            ...
            components = [
                components;
                Component(;
                    name = "bld$(ibld)_strut$istrut",
                    nodeNumbers = collect(s2t_idx[ibld, istrut]:s2b_idx[ibld, istrut]),
                )
            ]
    ...
    if Nguy_sets>0
        ...
        for iguy_vert = 1:Nguy_vert
            for iguy_set = 1:Nguy_sets
                ...
                g2t_idx[iguy_set, iguy_vert],
                g2ground_idx[iguy_set, iguy_vert],
                mesh_x,
                mesh_y,
                mesh_z,
                conn,
                ...
                components = [
                    components;
                    Component(;
                        name = "guy_vert$(iguy_vert)_guy_set$iguy_set",
                        nodeNumbers = collect(
                            g2t_idx[iguy_set, iguy_vert]:g2ground_idx[iguy_set, iguy_vert],
                        ),
                    )
                ]
    ...
    for ibld = 1:nblade
        ...
        if !prebend
            for istrut = 1:Nstrut
                ...
                jointtype[joint_idx] = strut_tower_joint_type #fixed
                Psi_d_joint_offset[joint_idx] = 0 #no offset
                Theta_d_joint_offset[joint_idx] = 0 #no offset
        ...
        for istrut = 1:Nstrut
            ...
            jointtype[joint_idx] = strut_blade_joint_type # about second axis (y in blade local coordinates which is about the chord) This is to allow for hinged pitched blades 
    ...
    for ibld = 1:nblade
        myort.Psi_d[(end-ibld*nselem+1):(end-(ibld-1)*nselem)] .=
            myort.Psi_d[end-ibld*nselem+1]
        myort.Theta_d[(end-ibld*nselem+1):(end-(ibld-1)*nselem)] .=
            myort.Theta_d[end-ibld*nselem+1]
        myort.Twist_d[(end-ibld*nselem+1):(end-(ibld-1)*nselem)] .=
            myort.Twist_d[end-ibld*nselem+1]
    end
    ...
    #Joint Number,   Joint Connections, Joint Type, Joint Mass, Not Used, Psi_D, Theta_D
    myjoint =
        [Float64.(1:1:njoint) jointconn jointtype zeros(njoint) jointnames Psi_d_joint .+
                                                                           Psi_d_joint_offset Theta_d_joint .+
                                                                                              Theta_d_joint_offset]
...
function create_arcus_mesh(;
    ...
    #Joint Number,   Joint Connections, Joint Type, Joint Mass, Not Used, Psi_D, Theta_D
    myjoint =
        [Float64.(1:1:njoint) jointconn zeros(njoint) .+ joint_type zeros(njoint) zeros(
            njoint,
        ) Psi_d_joint Theta_d_joint]
    ...
    for i = 1:numEl #loop over elements
        ...
        if isapprox(v[1], 0.0; atol = 1e-5) && isapprox(v[2], 0.0; atol = 1e-5) #If we are vertical, correct for gimbal lock
            ...
            elseif abs(dotTest)<1.0e-4
                twist_dtemp = 90.0
                nVec1, nVec2, nVec3 = rigidBodyRotation(
                    0,
                    0,
                    1,
                    [Psi_d[i], Theta_d[i], twist_dtemp],
                    [3, 2, 1],
                )
                nVec = [nVec1 nVec2 nVec3]
...
function calculatePsiTheta(v)
    ...
    # avoid gimbal lock
    if isapprox(v[2], 0.0; atol = eps(), rtol = 0) &&
       isapprox(v[1], 0.0; atol = eps(), rtol = 0)
...
function getOWENSPreCompOutput(numadIn; yscale = 1.0, plyprops = plyproperties())
    ...
    lam_U = Array{Composites.Laminate,2}(
        undef,
        n_stations,
        sum((numadIn.segments[1, :] .> 0.0)[2:end]),
    )
    lam_L = Array{Composites.Laminate,2}(
        undef,
        n_stations,
        sum((numadIn.segments[1, :] .<= 0.0)[2:end]),
    )
    ...
    for i_station = 1:n_stations
        ...
        af_xy = DelimitedFiles.readdlm(
            "$(numadIn.airfoil[i_station]).csv",
            ',',
            Float64,
            skipstart = 0,
        )
        ...
        ##################################
        xsec_nodeU =
            Float64.(numadIn.segments[i_station, numadIn.segments[i_station, :] .>= 0.0])
        ...
        xsec_nodeL = Float64.(
            abs.(
                reverse(
                    numadIn.segments[i_station, numadIn.segments[i_station, :] .<= 0.0],
                ),
            ),
        ) #TODO: fix types and verify positive increasing is correct for precomp even on the bottom
        ...
        for i_af = 1:length(xnode)
            ...
            if length(loc_web)>=1
                for j_web = 1:length(loc_web)
                    if !isapprox(xnode[i_af], loc_web[j_web], atol = 1e-4) &&
                       alreadyPushed == false
        ...
        precompinput[i_station] = OWENSPreComp.Input(
            convert(TI, normalchord[i_station]),
            convert(TI, -twist_d[i_station]),
            convert(TI, -twistrate_d[i_station]),
            convert(TI, leloc[i_station]),
            convert(VTI, xnode_filtered),
            convert(VTI, ynode_filtered),
            convert(VTI, e1),
            convert(VTI, e2),
            convert(VTI, g12),
            convert(VTI, anu12),
            convert(VTI, density),
            convert(VTI, xsec_nodeU),
            n_laminaU,
            convert(VTI, n_pliesU),
            convert(VTI, t_lamU),
            convert(VTI, tht_lamU),
            mat_lamU,
            convert(VTI, xsec_nodeL),
            n_laminaL,
            convert(VTI, n_pliesL),
            convert(VTI, t_lamL),
            convert(VTI, tht_lamL),
            mat_lamL,
            convert(VTI, loc_web),
            n_laminaW,
            convert(VTI, n_pliesW),
            convert(VTI, t_lamW),
            convert(VTI, tht_lamW),
            mat_lamW,
        )
        ...
        for i_lam = 1:length(n_laminaU)
            ...
            lam_U[i_station, i_lam] = Composites.Laminate(
                mat_lamU[start_idx:end_idx],
                floor.(Int, n_pliesU[start_idx:end_idx]),
                t_lamU[start_idx:end_idx],
                tht_lamU[start_idx:end_idx],
            )
            start_idx = end_idx+1
        end
        start_idx = 1
        for i_lam = 1:length(n_laminaL)
            ...
            lam_L[i_station, i_lam] = Composites.Laminate(
                mat_lamL[start_idx:end_idx],
                floor.(Int, n_pliesL[start_idx:end_idx]),
                t_lamL[start_idx:end_idx],
                tht_lamL[start_idx:end_idx],
            )
            start_idx = end_idx+1
        end
        start_idx = 1
        for i_lam = 1:length(n_laminaW)
            ...
            lam_W[i_station, i_lam] = Composites.Laminate(
                mat_lamW[start_idx:end_idx],
                floor.(Int, n_pliesW[start_idx:end_idx]),
                t_lamW[start_idx:end_idx],
                tht_lamW[start_idx:end_idx],
            )
            start_idx = end_idx+1
        end
    end
...
"""
function getSectPropsFromOWENSPreComp(
    usedUnitSpan,
    numadIn,
    precompoutput;
    GX = false,
    precompinputs = nothing,
    fluid_density = 0.0,
    ...
    for i = 1:(length(usedUnitSpan)-1)
        ...
        sectionPropsArray[i].a =
            (
                sectionPropsArray[i].a .+ 0.25*2*sectionPropsArray[i].b .-
                sectionPropsArray[i].b
            ) ./ sectionPropsArray[i].b
        ...
        sectionPropsArray[i].aeroCenterOffset =
            sectionPropsArray[i].ac .* sectionPropsArray[i].b .- sectionPropsArray[i].a
...
function create_hawt_biwing_mesh(;
    ...
    for ibld = 1:nblade
        ...
        for isec = 1:N_sec
            ...
            mesh_x =
                [mesh_x; bldsec_Y[isec] .* sin(myangle) .+ bldsec_X[isec] .* cos(myangle)]
            mesh_y =
                [mesh_y; bldsec_X[isec] .* sin(myangle) .+ bldsec_Y[isec] .* cos(myangle)]
            ...
            if isec == 1
                ...
                b_sectopidx[ibld, isec] =
                    b_sectopidx[ibld, isec-1] + 1 + length(bldsec_Z[isec])-1#*ibld-1
            end
    ...
    for iblade = 1:nblade
        ...
        # Node Numbers
        structuralNodeNumbers[iblade, :] =
            collect(b_secbotidx[iblade, 1]:b_sectopidx[iblade, 4])
