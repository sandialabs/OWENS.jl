function [freqMat] = campDiagramGen(inputFileName,outputFileName,rotorSpeedArray,spinUpOn,numModes)
...
# **********************************************************************
#   [freqMat] = campDiagramGen(inputFileName,outputFileName,...
...
# 
#      output:
#      freqMat               = n x m array of modal frequencies (Hz) for m
#                           modes at n rotor speeds
    
    freqMat = zeros(length(rotorSpeedArray),numModes);

    for i=1:length(rotorSpeedArray)
        rotorSpeed = rotorSpeedArray(i);
        if(i>1)
            load displnl;
        [freqMat(i,:),~] = owens(inputFileName,'M',rotorSpeed,spinUpOn,numModes,displ);
        else
            [freqMat(i,:),~] = owens(inputFileName,'M',rotorSpeed,spinUpOn,numModes);
        end
    end

    save(outputFileName,'freqMat','rotorSpeedArray');
end