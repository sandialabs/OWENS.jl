...
function owens_to_gx(mymesh,myort,myjoint,sectionPropsArray,stiff_array,mass_array;VTKmeshfilename=nothing,damp_coef=0.05,inf_stiff=false)
    ...
    damping = fill([damp_coef,damp_coef,damp_coef,damp_coef,damp_coef,damp_coef], length(mass))
    ...
    for ipt = 1:length(assembly.points)
        ...
        myaf = frames2*[zero(xaf[elNum,:]) (xaf[elNum,:].-maximum(xaf[elNum,:])/2) yaf[elNum,:]]'
...
function OWENSVTK(saveName,t,uHist,system,assembly,sections,aziHist,mymesh,myel,
    ...
    OWENS.OWENSFEA_VTK(saveName,t[it_save],uHist[:,[it_save]],system,assembly,sections;scaling=1,azi[it_save],userPointNames,userPointData)
...
function OWENSFEA_VTK(filename,tvec,uHist,system,assembly,sections;
    ...
    for isave = 1:length(tvec)
        ...
        for ipoint = 1:length(points)
            ...
            u = SVector(Float64(disp_matrix[ipoint,1]),Float64(disp_matrix[ipoint,2]),Float64(disp_matrix[ipoint,3]))
            udot = SVector(0.0,0.0,0.0)
            theta = SVector(Float64(curv_matrix[ipoint,1]),Float64(curv_matrix[ipoint,2]),Float64(curv_matrix[ipoint,3]))
            ...
            points[ipoint] = GXBeam.PointState(u,#::SVector{3, TF}
            udot,
            theta,#::SVector{3, TF}
            thetadot,
            V,
        ...
        for ielem = 1:length(elements)
            ...
            elements[ielem] = GXBeam.ElementState(u,
            udot,
            theta,
            thetadot,
            V,
...
function mywrite_vtk(name, assembly, state; sections = nothing, scaling=1.0,
    metadata=Dict())
    ...
    paraview_collection(name) do pvd
        ...
        for (current_step, state) in enumerate(history)
            ...
            for i = 1:3
                data = Matrix{eltype(assembly)}(undef, 3, nelem)
                for ielem = 1:nelem
                    CtCab = GXBeam.wiener_milenkovic(state.elements[ielem].theta)' * assembly.elements[ielem].Cab
                    data[:, ielem] .= CtCab * axis_vector[i]
                end
                vtkfile[axis_name[i], VTKCellData()] = data
            end
...
"""
function mywrite_vtk(name, assembly, state, λ, eigenstate;
    sections = nothing,
    scaling=1.0,
    mode_scaling=1.0,
    cycles = 1,
    ...
    paraview_collection(name) do pvd
        ...
        for (current_step, t) in enumerate(time)
            ...
            for i = 1:3
                data = Matrix{eltype(assembly)}(undef, 3, nelem)
                for ielem = 1:nelem
                    CtCab = GXBeam.wiener_milenkovic(real(state.elements[ielem].theta .* exp(λ*t)))' *
