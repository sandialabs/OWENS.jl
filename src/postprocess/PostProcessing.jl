...
function my_getABD!(
    ...
    nply::AbstractArray{<:Integer,1},
    tply::AbstractArray{<:Real,1},
    theta::AbstractArray{<:Real,1},
...
"""
function my_getplystrain(
    nply::AbstractArray{<:Integer,1},
    tply::AbstractArray{<:Real,1},
    theta::AbstractArray{<:Real,1},
    resultantstrain::AbstractArray{<:Real,1},
    offset = zeros(3),
)
    ...
    localstrain[1, :] = [
        resultantstrain[1]+z[k]*resultantstrain[5]+offset[2]*resultantstrain[6] for
        k = 1:(nlam+1)
    ]
...
function sort_peaks(signal::AbstractVector{T}, dt = collect(1.0:length(signal))) where {T}
    ...
    is_extremum = vcat(true, (slope[1:(end-1)] .* slope[2:end]) .<= 0.0, true)
...
show(io::IO, x::Cycles_bounds) = print(
    io,
    "Cycles_bounds : min mean value=",
    x.min_mean,
    ", max mean value=",
    x.max_mean,
    ", max range=",
...
function sum_cycles(
    cycles::Array{Cycle,1},
    range_intervals::Interval{T},
    mean_intervals::Interval{T},
) where {T<:Real}
    bounds = find_boundary_vals(cycles)
    bins = zeros(
        typeof(cycles[1].count),
        length(range_intervals) - 1,
        length(mean_intervals) - 1,
    )
...
function rainflow(signal; nbins_range = 10, nbins_mean = 10, m = 3, Teq = 1)
    ...
    Ncycles, meanIntervals, rangeIntervals =
        sum_cycles(cycles, range_intervals_percent, mean_intervals_perc) # Sums the cycles in the given intervals
...
function calcSF(
    ...
    topstrainout = zeros(
        ...
        length(lam_in[1, :]),
        9,
    ) # time, span, lam, x,y, Assumes you use zero plies for sections that aren't used
    ...
    for i_station = 1:lencomposites_span
        ...
        if upper # Get laminate location starting and ending points
            lam_y_loc_le =
                precompinput[i_station].xsec_nodeU .* precompinput[i_station].chord
        else
            lam_y_loc_le =
                precompinput[i_station].xsec_nodeL .* precompinput[i_station].chord
        end
        ...
        thickness_precomp_lag_te =
            (precompinput[i_station].chord-(refY+precompoutput[i_station].y_sc))
        ...
        for j_lam = 1:length(lam_in[i_station, :])
            ...
            idx_y = round(
                Int,
                lam_y_loc_le[j_lam]/precompinput[i_station].chord*length(
                    precompinput[i_station].ynode,
                )/2,
            )

            if upper
                thickness_precomp_flap =
                    precompinput[i_station].ynode[idx_y]*precompinput[i_station].chord -
                    precompoutput[i_station].x_sc
                ...
                thickness_precomp_flap =
                    precompinput[i_station].ynode[end-idx_y+1]*precompinput[i_station].chord -
                    precompoutput[i_station].x_sc
            end
            ...
            stress_eachlayer =
                zeros(eltype(eps_x), length(eps_x[:, 1]), length(lam.nply), 3) #all timesteps for each stack layer
            ...
            for its = 1:length(eps_x[:, 1])
                ...
                lowerplystrain, upperplystrain =
                    my_getplystrain(lam, resultantstrain, offset)
                ...
                if failmethod == "maxstress"
                    try
                        SF_ult[its, i_station, j_lam] = minimum(
                            minimum([sf[isf][sf[isf] .> 0.0] for isf = 1:length(sf)]),
                        ) # Pick out the worst case safety factor that is positive - negative means it is in the wrong direction for the failure criteria
                    catch
                    end
                else
                ...
                b =
                    (
                        numadIn.segments[
                            i_station,
                            j_lam+2,
                        ]-numadIn.segments[i_station, j_lam+1]
                    )*numadIn.chord[i_station]
                R = numadIn.chord[i_station]/2
                laminate_h =
                    sum(precompinput[i_station].t_lamU .* precompinput[i_station].n_pliesU)
                plate_uniaxial_local_buckling_stress =
                    Composites.plate_uniaxial_local_buckling(A, D, b)
                plate_bending_local_buckling_stress =
                    Composites.plate_bending_local_buckling(D, b)
                plate_shear_local_buckling_stress =
                    Composites.plate_shear_local_buckling(A, D, b)

                weakest_buck = minimum([
                    plate_uniaxial_local_buckling_stress,
                    plate_bending_local_buckling_stress,
                    plate_shear_local_buckling_stress,
                ])
                compression_load =
                    min(stress[its, i_station, j_lam, 1], stress[its, i_station, j_lam, 2]) # if they are both negative, then it will give the worse compression load
            ...
            if calculate_fatigue
                ...
                for ilayer = 1:length(lam.nply)
                    ...
                    damage_layers[ilayer] =
                        fatigue_damage(
                            stressForFatigue,
                            SN_stress,
                            Log_SN_cycles2Fail1,
                            ultimate_strength,
                        )/total_t*60*60 #Damage to damage rate per hour
                end
...
function fatigue_damage(
    ...
    ncycles, mean_bins, amplitude_bins, _ = rainflow(
        stress;
        nbins_range = nbins_amplitude,
        nbins_mean,
        m = wohler_exp,
        Teq = equiv_cycles,
    )
    ...
    if mean_correction
        ...
        effective_amplitude_levels = [
            iamplitude / (1 - imean / ultimate_strength) for
            iamplitude in amplitude_levels, imean in mean_levels
        ]
...
function printSF(
    ...
    if verbosity>0
        ...
        println(
            "At time $(idx1*delta_t)s at composite station $(idx2) of $(lencomposites_span) at lam $(idx3) of $(length(lam_used[idx2,:]))",
        )

        println("Maximum Damage per hr: $damageperhour")
        println(
            "At composite station $(idxdamage1) of $(lencomposites_span) at lam $(idxdamage2) of $(length(lam_used[idxdamage1,:]))",
        )
    end
    ...
    if !isempty(SF_buck[SF_buck .> 0.0])
        ...
        if verbosity>2
            ...
            println(
                "At time $(minbuck_sfidx[1]*0.05)s at composite station $(minbuck_sfidx[2]) of $(lencomposites_span) at lam $(minbuck_sfidx[3]) of $(length(lam_used[minbuck_sfidx[2],:]))",
            )
        end
        if verbosity>3
            println("Buckling")
    ...
    function printlamInfo(
        ...
        if verbosity>1
            println("\n$name SF min: $mymin")
            println(
                "At time $(idx1*delta_t)s at composite station $(idx2) of $(lencomposites_span)",
            )
...
function printsf_twr(
    ...
    if verbosity>0
        ...
        println(
            "At time $(idx[1]*delta_t)s at composite station $(idx[2]) of $(lencomposites_span) at lam $(idx[3]) of $(length(lam_twr[idx[2],:]))",
        )
        println("Maximum Damage per hr: $damageperhour")
        println(
            "At composite station $(idxdamage[1]) of $(lencomposites_span) at lam $(idxdamage[2]) of $(length(lam_twr[idxdamage[1],:]))",
        )
    ...
    if !isempty(SF_buck_T[SF_buck_T .> 0.0])
        ...
        if verbosity>3
            println("\nWorst buckling safety factor $(minbuck_sf)")
            println(
                "At time $(minbuck_sfidx[1]*0.05)s at composite station $(minbuck_sfidx[2]) of $(lencomposites_span) at lam $(minbuck_sfidx[3]) of $(length(lam_twr[minbuck_sfidx[2],:]))",
            )
        elseif verbosity>3
            println("Buckling")
            for istation = 1:lencomposites_span
...
function extractSF(
    ...
    kappa_x_hist_ps,
    epsilon_y_hist_ps;
    epsilon_x_hist_1 = nothing,
    kappa_y_hist_1 = nothing,
    kappa_z_hist_1 = nothing,
    epsilon_z_hist_1 = nothing,
    kappa_x_hist_1 = nothing,
    epsilon_y_hist_1 = nothing,
    verbosity = 2,
    usestationBld = 0,
    usestationStrut = 0,
    ...
    delta_t = 0.001,
    AD15bldNdIdxRng = nothing,
    AD15bldElIdxRng = nothing,
    strut_precompoutput = nothing,
    strut_precompinput = nothing,
    plyprops_strut = nothing,
    numadIn_strut = nothing,
    lam_U_strut = nothing,
    lam_L_strut = nothing,
    calculate_fatigue = true,
)
    ...
    if !isnothing(epsilon_x_hist_1)
        for ipt = 1:4
            for jts = throwawayTimeSteps:length(epsilon_x_hist_ps[1, 1, :])
                epsilon_x_hist[ipt, :, jts] =
                    epsilon_x_hist_ps[ipt, :, jts] + epsilon_x_hist_1[ipt, :, end]
                kappa_y_hist[ipt, :, jts] =
                    kappa_y_hist_ps[ipt, :, jts] + kappa_y_hist_1[ipt, :, end]
                kappa_z_hist[ipt, :, jts] =
                    kappa_z_hist_ps[ipt, :, jts] + kappa_z_hist_1[ipt, :, end]
                epsilon_z_hist[ipt, :, jts] =
                    epsilon_z_hist_ps[ipt, :, jts] + epsilon_z_hist_1[ipt, :, end]
                kappa_x_hist[ipt, :, jts] =
                    kappa_x_hist_ps[ipt, :, jts] + kappa_x_hist_1[ipt, :, end]
                epsilon_y_hist[ipt, :, jts] =
                    epsilon_y_hist_ps[ipt, :, jts] + epsilon_y_hist_1[ipt, :, end]
            end
        end
    else
        epsilon_x_hist[:, :, throwawayTimeSteps:end] =
            epsilon_x_hist_ps[:, :, throwawayTimeSteps:end]
        kappa_y_hist[:, :, throwawayTimeSteps:end] =
            kappa_y_hist_ps[:, :, throwawayTimeSteps:end]
        kappa_z_hist[:, :, throwawayTimeSteps:end] =
            kappa_z_hist_ps[:, :, throwawayTimeSteps:end]
        epsilon_z_hist[:, :, throwawayTimeSteps:end] =
            epsilon_z_hist_ps[:, :, throwawayTimeSteps:end]
        kappa_x_hist[:, :, throwawayTimeSteps:end] =
            kappa_x_hist_ps[:, :, throwawayTimeSteps:end]
        epsilon_y_hist[:, :, throwawayTimeSteps:end] =
            epsilon_y_hist_ps[:, :, throwawayTimeSteps:end]
    ...
    for ibld = 1:Nbld
        ...
        mesh_span_bld_el = safeakima(
            LinRange(0, 1, length(mesh_span_bld)),
            mesh_span_bld,
            LinRange(0, 1, stopE-startE+1),
        )
        composites_span_bld =
            numadIn_bld.span/maximum(numadIn_bld.span)*maximum(mesh_span_bld_el) #TODO: this assumes struts and blades are straight, should be mapped for all potential cases, also need to input hub offset

        for its = 1:N_ts
            # Interpolate to the composite inputs #TODO: verify node vs el in strain
            eps_x_bld[ibld, its, :] = safeakima(
                mesh_span_bld_el,
                epsilon_x_hist[1, startE:stopE, its],
                composites_span_bld,
            )
            eps_z_bld[ibld, its, :] = safeakima(
                mesh_span_bld_el,
                meanepsilon_z_hist[1, startE:stopE, its],
                composites_span_bld,
            )
            eps_y_bld[ibld, its, :] = safeakima(
                mesh_span_bld_el,
                meanepsilon_y_hist[1, startE:stopE, its],
                composites_span_bld,
            )
            kappa_x_bld[ibld, its, :] = safeakima(
                mesh_span_bld_el,
                kappa_x_hist[1, startE:stopE, its],
                composites_span_bld,
            )
            kappa_y_bld[ibld, its, :] = safeakima(
                mesh_span_bld_el,
                kappa_y_hist[1, startE:stopE, its],
                composites_span_bld,
            )
            kappa_z_bld[ibld, its, :] = safeakima(
                mesh_span_bld_el,
    ...
    composites_span_twr = safeakima(
        LinRange(0, 1, length(mesh_span_twr)),
        mesh_span_twr,
        LinRange(0, 1, length(twr_precompinput)),
    )
    for its = 1:N_ts
        # Interpolate to the composite inputs
        eps_x_twr[1, its, :] = safeakima(
            mesh_span_twr,
            epsilon_x_hist[1, start:stop, its],
            composites_span_twr,
        )
        eps_z_twr[1, its, :] = safeakima(
            mesh_span_twr,
            meanepsilon_z_hist[1, start:stop, its],
            composites_span_twr,
        )
        eps_y_twr[1, its, :] = safeakima(
            mesh_span_twr,
            meanepsilon_y_hist[1, start:stop, its],
            composites_span_twr,
        )
        kappa_x_twr[1, its, :] =
            safeakima(mesh_span_twr, kappa_x_hist[1, start:stop, its], composites_span_twr)
        kappa_y_twr[1, its, :] =
            safeakima(mesh_span_twr, kappa_y_hist[1, start:stop, its], composites_span_twr)
        kappa_z_twr[1, its, :] =
            safeakima(mesh_span_twr, kappa_z_hist[1, start:stop, its], composites_span_twr)
    ...
    topstrainout_blade_U, topDamage_blade_U = calcSF(
        total_t,
        stress_U,
        SF_ult_U,
        SF_buck_U,
        length(bld_precompinput),
        plyprops_bld,
        bld_precompinput,
        bld_precompoutput,
        lam_U_bld,
    ...
    printSF(
        ...
        SF_ult_U,
        SF_buck_U,
        composite_station_idx_U_bld,
        composite_station_name_U_bld,
        length(bld_precompinput),
        lam_U_bld,
        topDamage_blade_U,
        delta_t,
    ...
    topstrainout_blade_L, topDamage_blade_L = calcSF(
        total_t,
        stress_L,
        SF_ult_L,
        SF_buck_L,
    ...
    printSF(
        ...
        SF_ult_L,
        SF_buck_L,
        composite_station_idx_L_bld,
        composite_station_name_L_bld,
        length(bld_precompinput),
        lam_L_bld,
        topDamage_blade_L,
        delta_t,
    ...
    topstrainout_tower_U, topDamage_tower_U = calcSF(
        total_t,
        stress_TU,
        SF_ult_TU,
        SF_buck_TU,
        length(twr_precompinput),
        plyprops_twr,
        twr_precompinput,
        twr_precompoutput,
        lam_U_twr,
    ...
    topstrainout_tower_L, topDamage_tower_L = calcSF(
        total_t,
        stress_TL,
        SF_ult_TL,
        SF_buck_TL,
...
function quadgk_or_trapz(
    f::F,
    start::ForwardDiff.Dual,
    stop::ForwardDiff.Dual;
    atol,
) where {F}
    npoints = 1000
    err = nothing
...
struct PostProcessOptions
    ...
    usestationBld::Any
    usestationStrut::Any
    throwawayTimeSteps::Any
    calculate_fatigue::Any
end
...
function safetyfactor_fatigue(mymesh, components, delta_t; options = PostProcessOptions())
    ...
    for icomp = 1:size(components)[1]
        ...
        mesh_span_el = OWENS.safeakima(
            LinRange(0, 1, length(mesh_span)),
            mesh_span,
            LinRange(0, 1, stopE-startE+1),
        )
        ...
        topstrainout_U, topDamage_U = calcSF(
            total_t,
            stress_U,
            SF_ult_U,
            SF_buck_U,
        ...
        if usestationBld != 0
            println(
                "maximum $(components[icomp].name) stress: $(maximum(stress_U[:,usestationBld,8,1]))",
            )
            println(
                "minimum $(components[icomp].name) stress: $(minimum(stress_U[:,usestationBld,8,1]))",
            )
        end
        ...
        topstrainout_L, topDamage_L = calcSF(
            total_t,
            stress_L,
            SF_ult_L,
            SF_buck_L,
        ...
        if usestationBld != 0
            println(
                "maximum $(components[icomp].name) stress: $(maximum(stress_L[:,usestationBld,8,1]))",
            )
            println(
                "minimum $(components[icomp].name) stress: $(minimum(stress_L[:,usestationBld,8,1]))",
            )
        end
        OWENS.printSF(
            verbosity,
